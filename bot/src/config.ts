...
export const config = {
  ...
  ipc: {
    commandBridgePath: () => path.join(config.modsPath, "CommandBridge", "Saved"),
    commandsFile: () => path.join(config.ipc.commandBridgePath(), "commands.ndjson"),
    resultsFile: () => path.join(config.ipc.commandBridgePath(), "results.ndjson"),
    timeoutMs: 20_000,
    pollIntervalMs: 250,
  },
  ...
  heatmap: {
    positionsFile: () =>
      path.join(config.modsPath, "HeatmapCollector", "Saved", "positions.ndjson"),
